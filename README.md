...
## 📊 Hướng Dẫn Tích Hợp RSVP Với Google Sheets (Miễn Phí & Trực Tiếp)

Để thu thập thông tin xác nhận tham dự từ khách mời trực tiếp vào một file Excel/Google Sheets mà không cần qua bên thứ ba, bạn thực hiện 4 bước đơn giản sau:

### Bước 1: Tạo Google Sheet mới
1. Truy cập vào [Google Sheets](https://docs.google.com/spreadsheets).
2. Tạo một bảng tính mới và đặt tên (ví dụ: `Danh Sách Khách Mời Đám Cưới`).
3. Tạo các tiêu đề cột tại dòng đầu tiên (Hàng 1) từ cột A đến cột F như sau:
   - Cột A: `Thời gian`
   - Cột B: `Họ tên`
   - Cột C: `Số điện thoại`
   - Cột D: `Tham gia`
   - Cột E: `Số lượng khách`
   - Cột F: `Lời chúc`

### Bước 2: Thêm Google Apps Script
1. Trên thanh công cụ của Google Sheet, chọn **Tiện ích mở rộng (Extensions)** -> **Apps Script**.
2. Xóa toàn bộ mã code mặc định hiển thị trong khung soạn thảo.
3. Sao chép và dán toàn bộ đoạn code sau vào:
...
function doPost(e) {
  try {
    ...
    // Append data row: [Timestamp, Name, Phone, Attendance, GuestsCount, Wish]
    sheet.appendRow([
...
1. Ở góc trên cùng bên phải của trang Apps Script, nhấn nút **Triển khai (Deploy)** -> **Triển khai mới (New deployment)**.
2. Chọn loại triển khai: Click vào biểu tượng bánh răng bên cạnh "Select type" và chọn **Ứng dụng web (Web app)**.
3. Cấu hình các thông số sau:
   - **Mô tả**: `RSVP Wedding Web App`
   - **Thực thi dưới dạng (Execute as)**: Chọn **Tôi (Email của bạn)**.
   - **Ai có quyền truy cập (Who has access)**: Chọn **Bất kỳ ai (Anyone)**. *(Bước này rất quan trọng để khách mời gửi được form).*
4. Nhấn **Triển khai (Deploy)**.
5. Google sẽ yêu cầu ủy quyền tài khoản, bạn hãy làm theo các bước: chọn **Ủy quyền truy cập (Authorize access)** -> Chọn email của bạn -> Nhấn **Nâng cao (Advanced)** ở góc dưới -> Nhấn **Đi tới dự án không tên (Go to Untitled project (unsafe))** -> Nhấn **Cho phép (Allow)**.
6. Sau khi triển khai xong, sao chép đoạn mã **URL ứng dụng web (Web app URL)** được cung cấp (URL có dạng `https://script.google.com/macros/s/.../exec`).

### Bước 4: Cập nhật URL vào mã nguồn Website
1. Mở file `js/script.js` của dự án cưới.
2. Tìm dòng khai báo biến `RSVP_SCRIPT_URL` ở ngay đầu file (dòng 5):
   ```javascript
   const RSVP_SCRIPT_URL = "DÁN_URL_ỨNG_DỤNG_WEB_CỦA_BẠN_VÀO_ĐÂY";
   ```
3. Thay thế chuỗi trống bằng URL bạn vừa copy ở Bước 3.
4. Lưu file và đẩy code lên Github/Vercel của bạn! Giờ đây, mỗi khi khách bấm xác nhận, thông tin sẽ lập tiếp xuất hiện trên Google Sheet của bạn. 🎉
